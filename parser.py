import os
import requests
import time
from datetime import datetime
import urllib.parse
import re

# КОНФИГУРАЦИЯ (не зависит от дат)
TOKEN = os.getenv('TG_TOKEN', '7521422533:AAE2UaEpXpH8yh22gM2nAy3iQKg2EqAkYts')
CHAT = os.getenv('TG_CHAT', '819701342')

BOOKS = [
    "Путь одного агентства Торбосов",
    "Доказательная медицина талантов",
    "0,05 доказательная медицина Петр Талантов",
    "Ritz Carlton Шульце Хорст",
    "Стратегическое планирование Гарольд керцнер",
    "Благоволительницы литтелл",
    "Синдром Паганини",
    "Мастерство учителя лемов",
    "Масштаб Джеффри Уэст",
    "Человек на все рынки торп",
    "Стратегии гениев Дилтс",
    "Дневник реалиста Волочков",
    "Первые 20 часов Кауфман",
    "Принцип Абрамовича Дорофеев",
    "Укрощение амигдалы",
    "Пятая дисциплина Сенге",
    "Сложные решения хоровиц",
    "Как люди учатся Шеклтон",
    "Библиотека Сбербанка",
    "наказание наградой Альфи Кон",
    "инноваторы Айзексон Уолтер",
    "Парадокс перфекциониста",
    "Мотивация для творческих людей Макгиннесс",
    "как рождаются эмоции лиза барретт",
    "взлом креатива Майкл Микалко",
    "карточки рисовый штурм Майкл Микалко",
    "В поисках памяти кандель",
    "Расстроенная психика кандель",
    "кислород Ник Лэйн",
    "код Петцольд Чарльз",
    "почему я отвлекаюсь Эдвард Хэлловэлл",
    "принцип ставок энни дьюк",
    "сначала заплати себе Микаловиц",
    "элегия хиллбилли Джей Ди Вэнс",
    "спин продажи нил рекхэм",
    "Неприятие перемен Киган",
    "Эффективный руководитель питер друкер",
    "Время Березовского авен",
    "Биохакинг Олли Совиярви",
    "Память не изменяет наварро",
    "Инвестиции шарп",
    "О Шрифте эрик Шпикерманн",
    "мастер историй Пол Смит",
    "рома едет роман Свечников",
    "тишина тит нат хан",
    "бесстрашие тит нат хан",
    "lift Игорь Калинаускас",
    "в постели с врагом Хэл Вон",
    "главная книга основателя бизнеса Вассерман",
    "глазами физика левин уолтер",
    "год без покупок кейт фландерс",
    "ешь правильно беги быстро Скотт Джурек",
    "как это построено Маколи",
    "искусство сторителлинга Кармин Галло",
    "мастер слова Кармин Галло",
    "Шпион на миллиард долларов",
    "код дурова",
    "люди среди деревьев",
    "меняю жир на силу воли ярослав брин",
    "мясо с кровью энтони бурден",
    "на одной волне Эми Бэнкс",
    "огилви о рекламе Дэвид Огилви",
    "моя жизнь в рекламе Хопкинс",
    "ошибки на миллион долларов",
    "парадокс перфекциониста Тал Бен-Шахар",
    "слон на танцполе",
    "сотрудники на всю жизнь Лорейн Грабс-Уэст",
    "ультрамышление хэнк джон",
    "первые 90 дней майкл уоткинс",
    "18 минут брегман",
    "Со мной хотят общаться зверева",
    "Радость изнутри тан"
]
def send_telegram(text):
    """Универсальная отправка в Telegram"""
    url = f'https://api.telegram.org/bot{TOKEN}/sendMessage'
    try:
        r = requests.post(url, json={
            'chat_id': CHAT,
            'text': text,
            'parse_mode': 'HTML'
        }, timeout=5)
        return r.status_code == 200
    except:
        return False

def search_avito_today(book):
    """
    Универсальный поиск сегодняшних объявлений.
    Работает с любыми датами через параметр cd=1 Авито.
    """
    query = urllib.parse.quote(book)
    
    # МАГИЯ: cd=1 = "за сутки" - Авито сам фильтрует по текущей дате
    url = f"https://www.avito.ru/rossiya/knigi_i_zhurnaly?cd=1&q={query}&s=104"
    
    headers = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36',
        'Accept-Language': 'ru-RU,ru;q=0.9,en-US;q=0.8,en;q=0.7',
    }
    
    try:
        response = requests.get(url, headers=headers, timeout=10)
        
        # Проверка на капчу или блокировку
        if 'Доступ ограничен' in response.text or 'captcha' in response.text.lower():
            print(f"⚠️  Авито ограничил доступ для: {book}")
            return False, None, None
        
        # Проверка на отсутствие результатов
        if 'Нет подходящих объявлений' in response.text:
            return False, None, None
        
        # УНИВЕРСАЛЬНЫЙ ПАРСИНГ (работает с любой датой)
        # Ищем первый результат
        page = response.text
        
        # Вариант 1: новая структура Авито
        link_match = re.search(r'itemprop="url"[^>]*content="(https://www\.avito\.ru[^"]+)"', page)
        date_match = re.search(r'data-marker="item-date">([^<]+)<', page)
        
        # Вариант 2: старая структура (резерв)
        if not link_match:
            link_match = re.search(r'href="(https://www\.avito\.ru/[^"]+/\d+)"', page)
        if not date_match:
            date_match = re.search(r'class="[^"]*date[^"]*"[^>]*>([^<]+)<', page)
        
        if link_match and date_match:
            item_url = link_match.group(1)
            pub_date = date_match.group(1).strip()
            
            # ВАЖНО: Авито сам определяет "сегодня"
            # "сегодня" = всегда текущая дата календаря
            if 'сегодня' in pub_date.lower():
                return True, item_url, pub_date
        
        return False, None, None
        
    except Exception as e:
        print(f"Ошибка поиска {book}: {e}")
        return False, None, None

def main():
       """Вечный парсер (работает с любыми датами)"""
    current_datetime = datetime.now()
    date_str = current_datetime.strftime('%d.%m.%Y')
    time_str = current_datetime.strftime('%H:%M:%S')
    
    print(f"\n{'='*70}")
    print(f"🌐 ВЕЧНЫЙ ПАРСЕР | ДАТА: {date_str} | ВРЕМЯ: {time_str}")
    print(f"{'='*70}")
    
    found_count = 0
    
    # Проверка всех книг
    for i, book in enumerate(BOOKS, 1):
        status = f"[{i:2d}/{len(BOOKS)}] {book[:40]:<40}"
        
        found, url, pub_date = search_avito_today(book)
        
        if found:
            found_count += 1
            print(f"{status} ✅ СЕГОДНЯ ({pub_date})")
            
            # Уведомление (дата подставляется автоматически)
            message = (
                f"🆕 <b>НОВОЕ ОБЪЯВЛЕНИЕ {date_str}!</b>\n\n"
                f"📖 <b>{book}</b>\n"
                f"📅 {pub_date}\n"
                f"🔗 <a href='{url}'>Смотреть на Авито</a>\n"
                f"⏰ Найдено: {time_str}"
            )
            send_telegram(message)
            time.sleep(0.5)
        else:
            print(f"{status} 📭 нет новых")
        
        time.sleep(1)
    
    # Итог
    print(f"{'='*70}")
    print(f"📊 ИТОГ {date_str}: {found_count} новых из {len(BOOKS)} книг")
    print(f"{'='*70}")
    
    if found_count > 0:
        send_telegram(
            f"📊 <b>Отчёт {date_str}</b>\n"
            f"✅ Найдено: {found_count} новых объявлений\n"
            f"🔄 Следующая проверка через 5 минут"
        )

if __name__ == "__main__":
    main()
